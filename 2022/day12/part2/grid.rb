    ...
    def parse(input)
      ...
      array = input.split("\n").map.with_index do |line, row|
        line.chars.map.with_index do |letter, col|
          case letter
          when 'S' then Square.new(elevation: elevation['a'], row: row, col: col)
          when 'E' then start = Square.new(elevation: elevation['z'], row: row, col: col)
          else Square.new(elevation: elevation[letter], row: row, col: col)
          end
        end
      end

      new(array: array, start: start)
    end
