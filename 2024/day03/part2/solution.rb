...
module Solution
  ...
  class << self
    def call(input)
      ...
      allowed_expressions = "do()#{parsed_input}".split("don't()").flat_map do |substring|
        substring.split('do')[1..-1]
      end

      mul_instructions = allowed_expressions.join.scan(/mul\(\d{1,3},\d{1,3}\)/).map do |mul_instruction|
